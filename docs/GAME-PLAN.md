...
- F64 The Negotiator — trades items for passage via gesture language. AC: trade-state machine test. SHIPPED ✅ (evidence: test/negotiator-test.mjs 20/0 in gate v5+; engine model over injected offer/passage host, mount seam tracked in V1.1 debt ledger)
- F65 Child drawings — appear near playgrounds depicting events YOU caused. AC: event-grounding test. SHIPPED ✅ (evidence: test/childdrawings-test.mjs 17/0 in gate v5+; engine model over injected event ledger, render seam tracked in V1.1 debt ledger)
...
- F91 Staged wake cinematic — waking sequence procedurally staged per seed. AC: staging determinism test. SHIPPED ✅ (evidence: test/wakecinematic-test.mjs ALL PASS in gate v5; mounted game.ts — stageWakeCinematic/WakeCinematicPlayer; v1.1: shots now PLAYED at fresh-run start via story/wakemount.ts WakeMount — any input dismisses into the rise, motion-safety bypasses, test/wakemount-test.mjs 34/0 + PLAYTHROUGH_PASS PAGE_ERRORS=0)
...
- F95 Hardcore flicker battery UI — charge conveyed only by torch flicker (opt-in). AC: mode equivalence test. SHIPPED ✅ (evidence: test/flickerbattery-test.mjs FLICKERBATTERY ALL PASS 32 checks + test/f95-hardcore-toggle-test.mjs 33/33 ALL PASS; mounted src/core/game.ts mount batch B — FlickerBattery, and the v1.1 settings toggle is wired: HARDCORE BATTERY row in the VISUALS section → applyHardcoreBattery() → per-frame torch drive + HUD suppression)
