/*
 * Functional verification of the ambient difficulty hint system
 * (src/ui/hints.ts): pool selection thresholds, shift-gated rate limiting,
 * round-robin fragment picking, the 3-5 min jitter bounds, pure helpers,
 * and the DOM layer against a stub document.
 *
 * Run: node --experimental-strip-types test/hints-test.mjs
 */
...
check('poolFor maps cautiousness extremes to their pools', () => {
  assert.equal(poolFor(0), 'brave');
  assert.equal(poolFor(0.29), 'brave');
  assert.equal(poolFor(0.61), 'timid');
  assert.equal(poolFor(1), 'timid');
});

check('poolFor stays silent in the ambiguous middle', () => {
  for (const c of [BRAVE_THRESHOLD, 0.3, 0.45, 0.5, 0.6, TIMID_THRESHOLD]) {
    assert.equal(poolFor(c), null, 'cautiousness ' + c + ' must be silent');
  }
});

check('poolFor treats junk input as silence, not a crash', () => {
  assert.equal(poolFor(NaN), null);
});

check('thresholds leave an ambiguous band between the pools', () => {
  assert.ok(BRAVE_THRESHOLD < TIMID_THRESHOLD);
  assert.ok(TIMID_THRESHOLD - BRAVE_THRESHOLD >= 0.2);
});
